    ...
    <div className="space-y-8">
      ...
      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{delay:.05}} className="p-8 rounded-3xl bg-white dark:bg-gray-800 border border-gray-200/50 dark:border-gray-700/50 shadow">
        ...
        <div className="mt-4 flex gap-3">
          <button onClick={enableAutoPay} className="px-5 py-3 rounded-xl bg-gradient-to-r from-[#3b82f6] to-[#06b6d4] text-[#0a0c19] font-semibold">{safeTenant.autoPayEnabled ? 'Manage Auto‑Pay' : 'Enable Auto‑Pay'}</button>
      ...
      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{delay:.1}} className="p-8 rounded-3xl bg-white dark:bg-gray-800 border border-gray-200/50 dark:border-gray-700/50 shadow">
        ...
        <motion.button onClick={()=>setShowPayModal(true)} whileHover={{scale:1.02}} whileTap={{scale:.98}} className="w-full px-8 py-6 bg-gradient-to-r from-[#3b82f6] to-[#06b6d4] text-[#0a0c19] font-bold text-xl rounded-2xl flex items-center justify-center gap-4">
            ...
            {safeHistory.slice(0, 6).map((p) => {
              ...
              <div key={p.id} className="flex items-center justify-between p-5 bg-gray-50 rounded-2xl">
                ...
                <div className="text-right">
                  <span className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-[#3b82f6]/10 to-[#06b6d4]/10 text-[#3b82f6] font-semibold rounded-xl border border-[#3b82f6]/30">✅ Paid</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-xl mx-4 p-6 rounded-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-xl">
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button disabled={isProcessing} onClick={handleConfirmPay} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#3b82f6] to-[#06b6d4] text-[#0a0c19] font-semibold disabled:opacity-60">{isProcessing ? 'Processing…' : 'Confirm & Pay'}</button>
            </div>
          </div>
        </div>
      )}
