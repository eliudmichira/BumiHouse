        ...
        <div className="space-y-6">
            <div className={`p-5 rounded-2xl ${isDark ? 'bg-[#10121e]' : 'bg-white shadow-sm'}`}>
                <h2 className={`text-xl font-bold mb-2 flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    <Camera className="w-6 h-6 text-[#3b82f6]" /> Property Images
                </h2>
                ...
                {/* Main Upload Button */}
                <label className={`w-full aspect-video rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all mb-6 ${isDark ? 'border-gray-700 bg-[#1a1d2d] hover:bg-[#252a40] hover:border-[#3b82f6]/50' : 'border-gray-300 bg-gray-50 hover:bg-gray-100 hover:border-[#3b82f6]/50'}`}>
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 ${isDark ? 'bg-[#3b82f6]/10' : 'bg-[#3b82f6]/10'}`}>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {previewImages.map((img, i) => (
                            <div key={i} className="aspect-square relative rounded-xl overflow-hidden group border border-gray-200 dark:border-gray-700">
                                ...
                                {i === 0 && (
                                    <span className="absolute bottom-1 left-1 bg-[#3b82f6] text-[#0a0c19] text-[10px] font-bold px-1.5 py-0.5 rounded">
                                        Cover
                                    </span>
                                )}
                            </div>
